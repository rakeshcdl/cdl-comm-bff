package com.cdl.escrow.serviceimpl;

import com.cdl.escrow.dto.ProcessingStatusDTO;
import com.cdl.escrow.entity.ProcessingStatus;
import com.cdl.escrow.exception.ApplicationConfigurationNotFoundException;
import com.cdl.escrow.mapper.ProcessingStatusMapper;
import com.cdl.escrow.repository.ProcessingStatusRepository;
import com.cdl.escrow.service.ProcessingStatusService;
import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.PageImpl;
import org.springframework.data.domain.Pageable;
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Transactional;

import java.util.Optional;

@Service
@RequiredArgsConstructor
@Slf4j
public class ProcessingStatusServiceImpl implements ProcessingStatusService {

    private final ProcessingStatusRepository repository;

    private final ProcessingStatusMapper mapper;


    @Override
    @Transactional(readOnly = true)
    public Page<ProcessingStatusDTO> getAllProcessingStatus(Pageable pageable) {
        log.debug("Fetching all processing status, page: {}", pageable.getPageNumber());
        Page<ProcessingStatus> page = repository.findAll(pageable);
        return new PageImpl<>(
                page.map(mapper::toDto).getContent(),
                pageable,
                page.getTotalElements()
        );
    }

    @Override
    @Transactional(readOnly = true)
    public Optional<ProcessingStatusDTO> getProcessingStatusById(Long id) {
        log.debug("Fetching processing status with ID: {}", id);
        return repository.findById(id)
                .map(mapper::toDto);
    }

    @Override
    @Transactional
    public ProcessingStatusDTO saveProcessingStatus(ProcessingStatusDTO processingStatusDTO) {
        log.info("Saving new processing status");
        ProcessingStatus entity = mapper.toEntity(processingStatusDTO);
        ProcessingStatus saved = repository.save(entity);
        return mapper.toDto(saved);
    }

    @Override
    @Transactional
    public ProcessingStatusDTO updateProcessingStatus(Long id, ProcessingStatusDTO processingStatusDTO) {
        log.info("Updating processing status with ID: {}", id);

        ProcessingStatus existing = repository.findById(id)
                .orElseThrow(() -> new ApplicationConfigurationNotFoundException("processing status not found with ID: " + id));

        // Optionally, update only mutable fields instead of full replacement
        ProcessingStatus toUpdate = mapper.toEntity(processingStatusDTO);
        toUpdate.setId(existing.getId()); // Ensure the correct ID is preserved

        ProcessingStatus updated = repository.save(toUpdate);
        return mapper.toDto(updated);
    }

    @Override
    @Transactional
    public Boolean deleteProcessingStatusById(Long id) {
        log.info("Deleting processing status with ID: {}", id);

        if (!repository.existsById(id)) {
            throw new ApplicationConfigurationNotFoundException("processing status not found with ID: " + id);
        }

        repository.deleteById(id);
        return true;
    }

    @Override
    @Transactional
    public boolean softDeleteProcessingStatusById(Long id) {
        return repository.findByIdAndDeletedFalse(id).map(entity -> {
            entity.setDeleted(true);
            repository.save(entity);
            return true;
        }).orElse(false);
    }
}
